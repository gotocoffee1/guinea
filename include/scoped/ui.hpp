...
struct Child
{
    ...
    Child(const char* str_id, const ImVec2& size = ImVec2(0, 0), ImGuiChildFlags child_flags = 0, ImGuiWindowFlags flags = 0)
    {
        IsContentVisible = ImGui::BeginChild(str_id, size, child_flags, flags);
    }
    Child(ImGuiID id, const ImVec2& size = ImVec2(0, 0), ImGuiChildFlags child_flags = 0, ImGuiWindowFlags flags = 0)
    {
        IsContentVisible = ImGui::BeginChild(id, size, child_flags, flags);
    }
    ~Child()
    {
        ImGui::EndChild();
